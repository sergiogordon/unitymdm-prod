...
async def deploy_apk(
    ...
    # Rate limiting: Limit concurrent bulk deployments to prevent overwhelming FCM
    rate_key = f"apk_deploy:{user.username}"
    if not apk_deploy_limiter.is_allowed(rate_key):
        raise HTTPException(
            status_code=429,
            detail="Rate limit exceeded. Maximum 5 bulk APK deployments per minute."
        )

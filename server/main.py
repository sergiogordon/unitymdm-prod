...
async def get_windows_one_liner_script(
    ...
    one_liner = f'''cmd.exe /K "echo ============================================ & echo UnityMDM Zero-Tap Enrollment v3 - {alias} & echo ============================================ & echo. & echo [Step 0/10] Check prerequisites... & where adb && echo [OK] ADB found & echo. & echo [Step 1/10] Wait for device... & adb wait-for-device && echo [OK] Device connected & adb devices -l & echo. & echo [Step 2/10] Check Android version... & adb shell getprop ro.build.version.release & adb shell getprop ro.build.version.sdk & echo. & echo [Step 3/10] Download APK... & curl -L -H "X-Admin-Key: {admin_key}" "{server_url}/v1/apk/download-latest" -o {apk_path} && echo [OK] APK downloaded & echo. & echo [Step 4/10] Install APK... & adb install -r -g {apk_path} || adb uninstall {agent_pkg} && adb install -r -g -t {apk_path} & echo. & echo [Step 5/10] Verify package installed... & adb shell pm path {agent_pkg} & echo. & echo [Step 6/10] Set Device Owner... & adb shell dpm set-device-owner {agent_pkg}/.NexDeviceAdminReceiver & echo. & echo [Step 7/10] Grant permissions... & adb shell pm grant {agent_pkg} android.permission.POST_NOTIFICATIONS & adb shell pm grant {agent_pkg} android.permission.ACCESS_FINE_LOCATION & adb shell appops set {agent_pkg} RUN_ANY_IN_BACKGROUND allow & adb shell appops set {agent_pkg} GET_USAGE_STATS allow & adb shell dumpsys deviceidle whitelist +{agent_pkg} & echo [OK] Permissions granted & echo. & echo [Step 8/10] Apply system tweaks... & adb shell settings put global app_standby_enabled 0 & echo [OK] System tweaks applied & echo. & echo [Step 9/10] Auto-enroll and launch... & adb shell am broadcast -a com.nexmdm.CONFIGURE -n {agent_pkg}/.ConfigReceiver --receiver-foreground --es server_url {server_url} --es admin_key {admin_key} --es alias {alias} & adb shell monkey -p {agent_pkg} -c android.intent.category.LAUNCHER 1 & echo [OK] App launched & echo. & echo [Step 10/10] Verify service... & timeout /t 5 /nobreak >nul & adb shell pidof {agent_pkg} & echo. & echo ============================================ & echo Enrollment complete for {alias} & echo ============================================"'''

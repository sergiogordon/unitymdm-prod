...
async def deploy_apk(
    ...
    batch_size: int = Query(default=10, ge=1, le=50, description="FCM dispatch batch size"),
    batch_delay_ms: int = Query(default=500, ge=0, le=5000, description="Delay between batches in milliseconds")
):
    """
    Deploy an APK to selected devices via FCM push notification with batched dispatch.

    Batching prevents Firebase quota exhaustion and reduces backend load spikes.
    Default: 10 devices per batch with 500ms delay between batches.

    For 50 devices: 5 batches x 500ms = 2.5s total dispatch time
    For 100 devices: 10 batches x 500ms = 5s total dispatch time
    """
    verify_admin_key_header(credentials)

    apk_version = db.query(ApkVersion).filter(ApkVersion.id == request.apk_id).first()
    if not apk_version:
        raise HTTPException(status_code=404, detail="APK not found")

    target_device_ids = request.device_ids if request.device_ids else []

    if not target_device_ids:
        all_devices = db.query(Device).filter(Device.fcm_token.isnot(None)).all()
    ...
    for batch_idx in range(0, len(target_device_ids), batch_size):
        batch = target_device_ids[batch_idx:batch_idx + batch_size]

        structured_logger.log_event(
            "apk.deploy.batch_start",
            level="INFO",
            ...
            fcm_message = {
                "message": {
                    ...
                    "data": {
                        ...
                        "ts": timestamp,
                        "hmac": hmac_signature,
                        "apk_id": str(apk_version.id),
                        "version_name": apk_version.version_name,
                        "version_code": str(apk_version.version_code)
                    },
                    "android": {
                        "priority": "high"
                    }
        ...
        # Delay between batches to prevent quota exhaustion
        if batch_idx + batch_size < len(target_device_ids) and batch_delay_ms > 0:
        ...
        "installations": [
            {
                ...
                "device": {
                    "id": inst.device_id,
                    "alias": db.query(Device).filter(Device.id == inst.device_id).first().alias
                }
            }

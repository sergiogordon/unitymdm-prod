...
async def get_windows_one_liner_script(
    ...
    unity_apk_path = "%TEMP%\\\\unity_app.apk"
    bloat_file = "%TEMP%\\\\mdm_bloatware.txt"
    
    one_liner = f'''cmd.exe /K "echo ============================================ & echo UnityMDM Zero-Tap Enrollment v4 - {alias} & echo ============================================ & echo. & echo [Step 0/12] Check prerequisites... & where adb & where curl & echo. & echo [Step 1/12] Wait for device... & adb wait-for-device & adb devices -l & echo. & echo [Step 2/12] Check Android version... & adb shell getprop ro.build.version.release & adb shell getprop ro.build.version.sdk & echo. & echo [Step 3/12] Download APKs (parallel)... & echo Starting parallel downloads... & start /b curl -L -s -H X-Admin-Key:{admin_key} {server_url}/v1/apk/download-latest -o {apk_path} & start /b curl -L -s -H X-Admin-Key:{admin_key} {server_url}/v1/apk/download-latest-unity -o {unity_apk_path} & echo Waiting for downloads... & timeout /t 15 /nobreak >nul & echo Download check: & dir {apk_path} 2>nul & dir {unity_apk_path} 2>nul & echo. & echo [Step 4/12] Install NexMDM APK... & adb install -r -g {apk_path} & echo. & echo [Step 5/12] Verify NexMDM installed... & adb shell pm path {agent_pkg} & echo. & echo [Step 6/12] Set Device Owner... & adb shell dpm set-device-owner {agent_pkg}/.NexDeviceAdminReceiver & echo. & echo [Step 7/12] Grant permissions... & adb shell pm grant {agent_pkg} android.permission.POST_NOTIFICATIONS & adb shell pm grant {agent_pkg} android.permission.ACCESS_FINE_LOCATION & adb shell appops set {agent_pkg} RUN_ANY_IN_BACKGROUND allow & adb shell appops set {agent_pkg} GET_USAGE_STATS allow & adb shell dumpsys deviceidle whitelist +{agent_pkg} & echo. & echo [Step 8/12] Apply system tweaks... & adb shell settings put global app_standby_enabled 0 & adb shell settings put global ambient_display_always_on 0 & adb shell cmd power set-adaptive-power-saver-enabled false & echo System tweaks applied. & echo. & echo [Step 9/12] Auto-enroll and launch NexMDM... & adb shell am broadcast -a com.nexmdm.CONFIGURE -n {agent_pkg}/.ConfigReceiver --receiver-foreground --es server_url {server_url} --es admin_key {admin_key} --es alias {alias} & timeout /t 2 /nobreak >nul & adb shell monkey -p {agent_pkg} -c android.intent.category.LAUNCHER 1 & echo. & echo [Step 10/12] Install Unity APK... & adb install -r -g {unity_apk_path} & echo. & echo [Step 11/12] Verify Unity installed... & adb shell pm path {unity_pkg} & echo. & echo [Step 12/12] Verify services... & timeout /t 3 /nobreak >nul & adb shell pidof {agent_pkg} & echo. & echo ============================================ & echo Enrollment complete for {alias} & echo ============================================"'''

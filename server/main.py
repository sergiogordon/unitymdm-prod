...
async def get_windows_one_liner_script(
    ...
    one_liner = f'''cmd.exe /K "echo ============================================ & echo UnityMDM Zero-Tap Enrollment v3 - {alias} & echo Supports: Android 11+ SDK 30+ & echo ============================================ & echo. & echo [Step 0/10] Check prerequisites... & where adb >nul 2>&1 && (echo [OK] ADB found & adb version 2>nul) || (echo [FAIL] ADB not found in PATH & echo Fix: Install Android Platform Tools & echo Download: developer.android.com/tools/releases/platform-tools) & echo Listing devices: & adb devices -l & echo. & echo [Step 1/10] Wait for device... & adb wait-for-device >nul 2>&1 && (echo [OK] Device connected) || (echo [FAIL] No device - Check USB cable & adb devices -l) & echo. & echo [Step 2/10] Check Android version... & echo Android version: & adb shell getprop ro.build.version.release & echo SDK level: & adb shell getprop ro.build.version.sdk & echo. & echo [Step 3/10] Download APK... & curl -L -H "X-Admin-Key: {admin_key}" "{server_url}/v1/apk/download-latest" -o {apk_path} >nul 2>&1 && (echo [OK] APK downloaded) || (echo [FAIL] Download failed - Check network) & echo. & echo [Step 4/10] Install APK... & adb install -r -g {apk_path} >nul 2>&1 && (echo [OK] APK installed) || (adb uninstall {agent_pkg} >nul 2>&1 & adb install -r -g -t {apk_path} >nul 2>&1 && echo [OK] APK installed || echo [FAIL] Install failed) & echo. & echo [Step 5/10] Set Device Owner... & adb shell dpm set-device-owner {agent_pkg}/.NexDeviceAdminReceiver 2>&1 & echo. & echo [Step 6/10] Grant permissions... & adb shell pm grant {agent_pkg} android.permission.POST_NOTIFICATIONS >nul 2>&1 & adb shell pm grant {agent_pkg} android.permission.ACCESS_FINE_LOCATION >nul 2>&1 & adb shell pm grant {agent_pkg} android.permission.CAMERA >nul 2>&1 & adb shell appops set {agent_pkg} RUN_ANY_IN_BACKGROUND allow >nul 2>&1 & adb shell appops set {agent_pkg} GET_USAGE_STATS allow >nul 2>&1 & adb shell dumpsys deviceidle whitelist +{agent_pkg} >nul 2>&1 & echo [OK] Permissions granted & echo. & echo [Step 7/10] Disable bloatware... & curl -s -H "X-Admin-Key: {admin_key}" "{server_url}/admin/bloatware-list" -o {bloat_file} >nul 2>&1 & echo Bloatware list downloaded & echo. & echo [Step 8/10] Apply system tweaks... & adb shell settings put global app_standby_enabled 0 >nul 2>&1 & adb shell settings put global battery_tip_constants app_restriction_enabled=false >nul 2>&1 & adb shell settings put system screen_brightness_mode 0 >nul 2>&1 & echo [OK] System tweaks applied & echo. & echo [Step 9/10] Auto-enroll and launch... & adb shell am broadcast -a com.nexmdm.CONFIGURE -n {agent_pkg}/.ConfigReceiver --receiver-foreground --es server_url {server_url} --es admin_key {admin_key} --es alias {alias} >nul 2>&1 & echo Broadcast sent & adb shell monkey -p {agent_pkg} -c android.intent.category.LAUNCHER 1 >nul 2>&1 & echo [OK] App launched & echo. & echo [Step 10/10] Verify service... & timeout /t 3 /nobreak >nul & adb shell pidof {agent_pkg} >nul 2>&1 && (echo [OK] Service running) || (echo [WARN] Service check - verify manually) & echo. & echo Verify registration... & echo Waiting 10 seconds for first heartbeat... & timeout /t 10 /nobreak >nul & echo Check dashboard for device {alias} & echo. & echo ============================================ & echo Enrollment script complete for {alias} & echo ============================================ & echo Window will stay open - Type exit to close"'''

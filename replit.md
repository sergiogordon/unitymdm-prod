...
- **APK Management (CI Integration)**: Admin dashboard for managing CI-built debug APKs, including registration, upload to Replit Object Storage, download, and deletion, all secured by admin key authentication. **Chunked Upload System**: Production-hardened upload reliability for APKs up to 500 MB using 5 MB chunks with automatic retry (3 attempts per chunk), real-time progress tracking, streaming backend writes, SHA-256 validation, and comprehensive logging to apk_uploads.log. Upload_id validation prevents path traversal attacks. Request size middleware exempts chunk endpoints. Manual uploads are hardcoded as "release" builds and automatically increment version_code on conflict to prevent duplicate errors.
...
- **Auth Self-Healing (Nov 2025)**: Automatic recovery from stale token scenarios where devices have expired authentication tokens (e.g., after server-side re-registration). QueueManager tracks consecutive 401 failures; after 5 consecutive auth failures, clears credentials and shows high-priority re-enrollment notification. ConfigReceiver now correctly saves device_id during registration. SecurePreferences tracks consecutive401Count and needsReEnrollment flags, with clearAuthCredentials() preserving serverUrl for easier re-enrollment. MonitorService implements AuthFailureListener interface to display alert notification when threshold is reached.
...
- **Heartbeat Performance Optimization (Dec 2025)**: Critical performance improvements for handling 50+ concurrent devices: (1) Eliminated duplicate `get_effective_monitoring_settings()` calls per heartbeat, (2) Replaced separate DeviceLastStatus SELECT+UPDATE with single UPDATE...RETURNING query, (3) Increased connection pool to 150 (75 base + 75 overflow) with 10s timeout for faster failure, (4) Removed debug print statements from hot path. Auth latency reduced to ~320-500ms, total heartbeat processing improved from 30s timeouts to 6-10s during burst traffic.

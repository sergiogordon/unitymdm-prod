...
export default function DemoDeviceOptimizationPage() {
  ...
  const loadWhitelist = async () => {
    try {
      const response = await DemoApiService.fetch('/v1/battery-whitelist')
  ...
  const handleAddToWhitelist = async () => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(`Added ${appName} to battery whitelist (demo mode)`)
  ...
  const handleRemoveFromWhitelist = async (id: number, appName: string) => {
    try {
      await DemoApiService.fetch(`/v1/battery-whitelist/${id}`, {
        method: 'DELETE'
      })
      toast.success(`Removed ${appName} from whitelist (demo mode)`)
      await loadWhitelist()
    } catch (error) {
  ...
  const handleApplyToFleet = async () => {
    setIsApplying(true)
    try {
      const response = await DemoApiService.fetch('/v1/devices/apply-battery-whitelist', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' }
      })
      const data = await response.json()
      toast.success(`Applied to all online devices (demo mode)`)
    } catch (error) {
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  Click "Apply to Fleet" to send FCM notifications to all online devices. Each device will receive the whitelist and request battery optimization exemption for all listed apps. 
                  Devices must be online and have FCM configured.
                </p>
              </div>
              <div>
                <h3 className="font-medium text-gray-900 dark:text-white mb-2">Android App Integration</h3>
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  The UNITYmdm Android app automatically fetches the whitelist from the server on startup and applies battery optimization exemptions for itself and all whitelisted apps. 
                  This ensures persistent protection across reboots.
                </p>
              </div>
            </div>
          </CardContent>
      ...
      <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Enter the package name and app name to exempt from battery optimization
            </DialogDescription>
          </DialogHeader>
